...
module.exports = {
  ...
  execute(message, args) {
    ...
    const volinfoembed = new MessageEmbed()
      .setColor("#FFB901")
      .setTitle(`<:Vol_:781591477305606164> | Volume is: \`${queue.volume}%\``);
    //if no args return info embed
    if (!args[0])
      return message.channel.send(volinfoembed).catch(console.error);
    ...
    const volinfosetembed = new MessageEmbed()
      .setColor("#FFB901")
      .setFooter(`Zooye | Created By , MONTY EXPERT`)
      .setTitle(
        `<:Vol_:781591477305606164> | Volume changed to: \`${args[0]}%\`!`
      );
