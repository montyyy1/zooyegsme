...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (!args.length)
      return attentionembed(
        message,
        `\`Usage:\` ${message.client.prefix}play (song title) - plays the first result from Youtube
${message.client.prefix}play (URL) - plays the provided song, playlist, or stream
\`Examples:\`
${message.client.prefix}play Alan Walker - Alone
${message.client.prefix}play https://youtu.be/1-xGerv5FOk`
      );
    //react with approve emoji
    message.react("<a:next:781695568744546304>").catch(console.error);
    //get permissions and send error if bot doesnt have enough
    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return attentionembed(
        message,
        "I need permissions to join your channel!"
      );
    if (!permissions.has("SPEAK"))
      return attentionembed(
        message,
        "I need permissions to speak in your channel"
      );
      ...
      if (serverQueue) {
        ...
        if (urlValid) {
          ...
          message.channel.send(
            new MessageEmbed()
              .setColor("#FFB901")
              .setDescription(
                `**<:770116524786581524:781661556722499586>  Searching <:FA38915F38474636A656C88C4C1D6309:781910843226587197>  [\`LINK\`](${args.join(
                  " "
                )})**`
              )
          );
          ...
          message.channel.send(
            new MessageEmbed()
              .setColor("#FFB901")
              .setDescription(
                `**<:770116524786581524:781661556722499586>  Searching <:FA38915F38474636A656C88C4C1D6309:781910843226587197>\`${args.join(
                  " "
                )}\`**`
              )
          );
        ...
        message.channel.send(
          new MessageEmbed()
            .setColor("#FFB901")
            .setDescription(
              `** <:740916473920356422:781648269356433408> Joined \`${channel.name}\` 📄 bound \`#${message.channel.name}\`**`
            )
            .setFooter(
    ...
    if (urlValid) {
      try {
        ...
        if (error.statusCode === 403)
          return attentionembed(
            message,
            "Max. uses of api Key, please refresh!"
          );
        console.error(error);
    ...
    //get the thumbnail
    let thumb =
      "https://cdn.discordapp.com/attachments/703781433646514197/769719204408459284/image1.png";
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        .setTitle("<a:763362089950511124:781695620372627477>  " + song.title)
        .setColor("#FFB901")
        .setThumbnail(thumb)
        .setURL(song.url)
        .addField(
