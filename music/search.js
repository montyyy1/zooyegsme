...
async execute(message,args,client) {
   ...
   if (!args.length)
    ...
    let resultsEmbed = new MessageEmbed()
      .setTitle("<:emoji_45:754548324845223956>  Results for: ")
    ...
    try {
      ...
      //react with 5 Numbers
        await resultsMessage.react("771980514403418122");
        await resultsMessage.react("771980546213150720");
        await resultsMessage.react("771980579989749810");
        await resultsMessage.react("771980618220306433");
        await resultsMessage.react("771980649187246100");
      ...
      {max: 1, time: 60000, errors: ['time'],} ).then(collected => {
        //if its one of the emoji set them to 1 / 2 / 3 / 4 / 5
          if(collected.first().emoji.id == "771980514403418122"){ return response = 1;}
          if(collected.first().emoji.id == "771980546213150720"){ return response = 2;}
          if(collected.first().emoji.id == "771980579989749810"){ return response = 3;}
          if(collected.first().emoji.id == "771980618220306433"){ return response = 4;}
          if(collected.first().emoji.id == "771980649187246100"){ return response = 5;}
